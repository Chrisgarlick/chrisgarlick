/**
 * Insert the 2026-05-07 trend blog post into the CMS database.
 * Converts the markdown to TipTap JSON and inserts with full SEO metadata.
 */
import { randomUUID } from 'crypto'
import postgres from 'postgres'

const sql = postgres('postgresql://cms:cms@localhost:5432/cms')

// -- TipTap JSON body (converted from blog.md) --
const body = {
  type: "doc",
  content: [
    p([
...
await sql`
  INSERT INTO articles (id, title, slug, body, excerpt, status, published_at, seo, created_at, updated_at)
  VALUES (
    ${id},
    ${"Why 79% of Enterprises Are Failing at AI Adoption"},
    ${"ai-adoption-challenges-2026"},
    ${JSON.stringify(body)},
    ${"79% of enterprises face AI adoption challenges despite record spending. Here's why most companies are stuck in pilot mode and what the top 20% do differently."},
    ${"published"},
...
console.log(`Slug: ai-adoption-challenges-2026`)
...
function link(text, href) {
  return { type: "text", text, marks: [{ type: "link", attrs: { href, target: null } }] }
}
