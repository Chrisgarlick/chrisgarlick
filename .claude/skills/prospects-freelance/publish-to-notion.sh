#!/bin/bash
# publish-to-notion.sh - Push freelance prospect list to Notion under Kritano
#
# Usage: ./publish-to-notion.sh <prospects.json> <page-title>
# Example: ./publish-to-notion.sh prospects.json "Freelance Prospects - Manchester - 2026-04-13"
...
echo "Publishing freelance prospects to Notion: $PAGE_TITLE"
...
blocks.append({
    ...
    'type': 'callout',
    'callout': {
        'icon': {'type': 'emoji', 'emoji': '\U0001f4bb'},
        'rich_text': [{'type': 'text', 'text': {'content': f'{len(prospects)} qualified freelancers from {total} discovered'}}]
    }
})
...
for p in prospects:
    ...
    # Freelancer signals
    signals = p.get('freelancer_signals', [])
    if signals:
        type_str = ', '.join(list(set(signals))[:3])
        details.append(f'Speciality: {type_str}')
    ...
    if social.get('github'):
        social_parts.append(f'GitHub: {social[\"github\"]}')
    if social.get('twitter'):
        social_parts.append(f'X: {social[\"twitter\"]}')
    if social.get('dribbble'):
        social_parts.append(f'Dribbble: {social[\"dribbble\"]}')
    if social.get('behance'):
        social_parts.append(f'Behance: {social[\"behance\"]}')
